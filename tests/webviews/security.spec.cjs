...
let server, base;
function fixture() {
  const finding = (id, category, severity, name) => ({ id, category, severity, confidence: 'medium', packageId: name, packageName: name, version: '1.0.0', location: `cache/${name}`, dependencyPath: ['app', name], ruleId: `rule-${id}`, title: 'Build script review', description: 'Investigate this build input.', recommendation: 'Inspect the evidence.', references: ['https://example.invalid/advisory'], evidence: [{ file: `cache/${name}/build.targets`, line: 2, column: 1, snippet: '<script>window.pwned = true</script>', lifecycle: `${name}:build`, chain: ['build', 'build.targets'] }] });
...
test('renders separate categories, partial coverage and escaped evidence', async ({ page }) => {
  ...
  await page.screenshot({ path: '.webview-test-results/report-webview.png', fullPage: true });
});
