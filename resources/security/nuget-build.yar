// DotNet CLI Plus NuGet build-script indicators, ruleset 1.0.0.
...
rule acp_process_api {
  meta:
    ...
    severity = "low"
    confidence = "low"
    description = "The package build scripts references process execution. Compilers and legitimate installers commonly do this."
  strings:
    $a = "child_process"
...
rule acp_download {
  meta:
    ...
    severity = "low"
    confidence = "low"
    description = "The package build scripts contains network retrieval syntax; downloading platform binaries is common in legitimate installers."
  strings:
    $a = /\b(curl|wget)\s+/
...
rule acp_encoded_execution {
  meta:
    ...
    severity = "moderate"
    confidence = "medium"
    description = "Encoding or decryption occurs alongside dynamic evaluation in the same package build input. Inspect whether decoded data becomes executable code."
  strings:
...
rule acp_download_execute {
  meta:
    ...
    severity = "high"
    confidence = "high"
    description = "Network retrieval is directly combined with interpreter execution. Review the remote source and the build command."
  strings:
...
rule acp_persistence {
  meta:
    ...
    severity = "high"
    confidence = "medium"
    description = "Package build input contains commands associated with persistence or disabling security controls. Verify the intended effect."
  strings:
...
rule acp_msbuild_inline_task {
  meta:
    title = "Inline MSBuild task"
    severity = "low"
    confidence = "low"
    description = "A package defines an inline MSBuild code task. Review the task body and target conditions; legitimate build tooling can also use inline tasks."
  strings:
    $task = /TaskFactory\s*=\s*["'](RoslynCodeTaskFactory|CodeTaskFactory)["']/ nocase
  condition: $task
}

rule acp_dotnet_encoded_execution {
  meta:
    title = "Encoded payload alongside .NET execution"
    severity = "high"
    confidence = "medium"
    description = "Base64 decoding occurs alongside assembly loading, process execution or PowerShell evaluation. Inspect whether the decoded payload becomes executable code."
  strings:
    $decode = "FromBase64String" nocase
    $exec1 = /Assembly\s*\.\s*Load/ nocase
    $exec2 = /Process\s*\.\s*Start/ nocase
    $exec3 = "Invoke-Expression" nocase
  condition: $decode and any of ($exec*)
}

rule acp_msbuild_exec {
  meta:
    title = "MSBuild command execution"
    severity = "low"
    confidence = "low"
    description = "A package build file invokes an external command. Compilers and legitimate build tools commonly do this; inspect the command and target conditions."
  strings:
    $exec = /<Exec[ \t\r\n]/ nocase
  condition: $exec
}
