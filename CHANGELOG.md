...
- **Microsoft.Testing.Platform (xUnit v3 native mode) support in the Test Explorer.** Projects referencing `Microsoft.Testing.Platform` or `xunit.v3` without `Microsoft.NET.Test.Sdk` are now first-class citizens: discovery runs `dotnet run --project X -- --list-tests`, the Run profile uses `--report-trx` (same TRX parsing/matching as VSTest, including theory display names), Debug builds the project and attaches the `coreclr` debugger to the test host dll directly, and Coverage uses MTP's `--coverage` flag (requires the `Microsoft.Testing.Extensions.CodeCoverage` package, referenced by the xunit.v3 templates by default) with the resulting cobertura parsed through the existing pipeline. Projects with neither Test.Sdk nor an MTP framework keep the explanatory placeholder item.
- **Coverage thresholds and diffs.** New settings `coverage.threshold.line` and `coverage.threshold.branch` (0–100, default 0 = disabled): when the Coverage profile finishes below a threshold, the run is marked failed with a synthetic "Coverage threshold" item stating actual vs. required percentages. Every coverage run now prints aggregate statement-weighted line/branch totals and a diff against the previous coverage run (stored per-workspace): overall ± percentage points, added/removed files and a worst-first list of files whose line coverage decreased (with lines lost). The `.NET: Clear Coverage Baseline` command resets the stored baseline.
...
- Microsoft.Testing.Platform coverage requires the `Microsoft.Testing.Extensions.CodeCoverage` package (referenced by the `xunit.v3` templates by default); without it, the Coverage profile reports that no cobertura file was produced.
