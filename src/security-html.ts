...
export function reportContent(report: SecurityReviewReport, standalone = false): string {
  ...
  <p>${escape(new Date(report.finishedAt).toLocaleString())} · ${report.packageCount} resolved packages · ${report.inputCount} script inputs · ${(report.inputBytes / 1048576).toFixed(1)} MiB</p>
  ${report.installOutcome ? `<p>Package operation result: ${escape(report.installOutcome)}. This review examines the resulting files.</p>` : ''}</div>
  ...
  <p>Package location: <code>${escape(finding.location)}</code></p><p>Dependency path: ${escape(finding.dependencyPath.join(' → '))}</p>
  ...
  ${finding.evidence.length ? `<h3>Evidence and build references</h3>${finding.evidence.map((evidence, index) => `<section class="evidence"><p><code>${escape(evidence.file)}:${evidence.line}:${evidence.column}</code>
...
export function buildSecurityHtml(options: { nonce: string; cspSource?: string; script: string; style: string; report?: SecurityReviewReport; standalone?: boolean }): string {
  ...
  <main id="report">${options.report ? reportContent(options.report, standalone) : '<p class="notice">Reading restored dependencies and NuGet build scripts…</p>'}</main>
