...
async function validRoot(root: string): Promise<string | undefined> {
  ...
  try { return await fs.realpath(root); } catch { return; }
}
function service(): SecurityReviewJobs {
  ...
  jobs = new SecurityReviewJobs(async (root, signal, installOutcome) => {
    ...
    return reviewPackageSecurity({ root, storage: context.globalStorageUri.fsPath,
      rules: vscode.Uri.joinPath(context.extensionUri, 'resources', 'security', 'nuget-build.yar').fsPath,
      signal, installOutcome,
      auditEnabled: vscode.workspace.getConfiguration('dotnetCliPlus', vscode.Uri.file(root)).get('securityReview.nugetAudit.enabled', true),
    ...
    void panels.get(root)?.panel.webview.postMessage({ type: 'idle', message: `Security review failed: ${String(error)}` });
...
export async function reviewPackageSecurityCommand(): Promise<void> {
  ...
  if (!folders.length) { vscode.window.showErrorMessage('Open a filesystem workspace to review restored packages.'); return; }
...
export async function beginSecurityInstall(root: string): Promise<string | undefined> {
  ...
  progress.set(real, 'Waiting for restore or package changes to finish…');
...
export function endSecurityInstall(root: string | undefined, outcome: 'success' | 'failed' | undefined): void {
  ...
  if (!service().busy(root)) {
    ...
    void panels.get(root)?.panel.webview.postMessage({ type: 'idle', message: 'Package operation finished. Run Rescan to review the current restored package files.' });
  }
}
export async function resolveEvidenceFile(root: string, report: SecurityReviewReport, findingId: string, index: number): Promise<{ file: string; line: number; column: number } | undefined> {
  ...
  const finding = report.findings.find(finding => finding.id === findingId);
  const evidence = finding?.evidence[index];
  if (!evidence) { return; }
  const absolute = path.resolve(root, evidence.file);
  // The allowed package location comes from our report, never from the webview message.
  const allowedRoot = finding?.location && path.isAbsolute(finding.location) ? finding.location : root;
  if (!contained(allowedRoot, absolute)) { return; }
  const file = await safeRealpath(allowedRoot, absolute);
