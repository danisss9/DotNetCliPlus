import type { DependencyEdge, DependencyGraph } from './nuget-graph';
...
export class DependencyGraphIndex {
  ...
  unresolvedIds(): Set<string> {
    ...
    return new Set(this.graph.edges.filter(edge => this.nodes.get(edge.target)?.status.includes('missing')).map(edge => edge.target));
  }
}
