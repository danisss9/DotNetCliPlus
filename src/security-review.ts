...
import { CATALOG_VERSION, packageFinding } from './security-catalog';
import { collectPackages } from './security-inventory';
import { discoverInstallInputs } from './security-scripts';
import { runNugetAudit } from './security-audit';
  ...
  for (const line of stdout.split(/\r?\n/).filter(line => line.trim())) {
    ...
    for (const raw of item.rules) {
      ...
      if (existing) { existing.evidence.push(...evidence); }
        ...
        description: meta.description, recommendation: 'Inspect the package build scripts and the reference chain. Confirm whether the behavior is expected before replacing or updating the dependency.', evidence,
...
export async function reviewPackageSecurity(options: ReviewOptions): Promise<SecurityReviewReport> {
  ...
  try {
    ...
    progress('inventory', 'Reading NuGet restore assets…');
    ...
    report.coverage.push({ component: 'catalog', state: 'skipped', checked: 0,
      messages: ['No curated NuGet malware catalog is bundled; live NuGet advisories and static script indicators are checked separately.'] });
    progress('nuget-audit', 'Checking live NuGet advisories…');
    const audit = await runNugetAudit(inventory.root, inventory.packages, options.auditEnabled, options.signal, options.auditRunner);
    report.coverage.push(audit.coverage); report.findings.push(...audit.findings);
    progress('scripts', 'Reading NuGet package build scripts…');
    ...
    if (!discovery.inputs.length) {
      report.coverage.push({ component: 'yara-x', state: 'complete', checked: 0, messages: ['No resolvable package build-script inputs to scan.'] });
    } else {
      ...
      progress('yara-x', `Scanning ${snapshots.size} package build-script inputs with YARA-X…`);
  ...
  for (const component of ['inventory', 'catalog', 'nuget-audit', 'scripts', 'yara-x'] as const) {
