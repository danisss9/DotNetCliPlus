...
import type { DependencyGraph, DependencyKind } from './nuget-graph';
...
function kindColor(kind: DependencyKind): string {
  return {
    package: color('--vscode-charts-blue', '#3794ff'),
    project: color('--vscode-charts-purple', '#b180d7'),
    framework: color('--vscode-charts-green', '#89d185'),
  }[kind];
}
...
function renderGraph(fit = false, resetPositions = false) {
  ...
  cy.batch(() => {
    ...
    for (const id of visible.nodes) {
      ...
      const data = { id, label: `${node.name}\n${node.version ?? 'unresolved'}`, color: kindColor(node.kinds[0] ?? 'package') };
  ...
  summary.textContent = `${index.graph.source} · ${visible.nodes.size - 1} of ${index.graph.nodes.length - 1} nodes visible · ${visible.edges.length} connections`;
...
function showDetails() {
  ...
  text('p', `Status: ${node.id === index.graph.root ? 'Project' : node.status.join(', ') || (index.graph.source === 'Restored' ? 'Restored' : 'Declared only')}`, details);
...
function searchPackages() {
  ...
  const unresolved = unresolvedOnly ? index.unresolvedIds() : undefined;
  const matches = index.graph.nodes.filter(node => {
    if (node.id === index!.graph.root || unresolved && !unresolved.has(node.id)) { return false; }
    if (query) {
      return `${node.name} ${node.version ?? ''} ${(index!.incoming.get(node.id) ?? []).map(edge => edge.name).join(' ')}`.toLowerCase().includes(query);
    }
    return unresolvedOnly || (index!.outgoing.get(index!.graph.root) ?? []).some(edge => edge.target === node.id);
  });
  element('unresolved').setAttribute('aria-pressed', String(unresolvedOnly));
  const notice = element('unresolved-notice');
  notice.hidden = !unresolvedOnly;
  notice.textContent = index.graph.source === 'Declared only'
    ? 'Unresolved dependencies cannot be checked from declarations alone. Restore packages, then Refresh.'
    : 'Showing unresolved dependencies in the current graph, including nested packages.  Use Refresh to check again.';
  results.replaceChildren();
  element('search-count').textContent = unresolvedOnly && index.graph.source === 'Declared only' ? 'Restore required'
    : `${matches.length} ${unresolvedOnly ? `${query ? 'matching ' : ''}unresolved ${matches.length === 1 ? 'dependency' : 'dependencies'}` : `${query ? 'matching dependencies' : 'projects'}`}${matches.length > 100 ? ' · showing first 100; refine your search' : ''}`;
...
function setBusy(value: boolean) {
  ...
  element<HTMLButtonElement>('unresolved').disabled = value || !index;
  updateExpandAll();
...
element('unresolved').addEventListener('click', () => {
...
const observer = new MutationObserver(() => {
  if (cy) {
    cy.nodes().forEach(node => { node.data('color', kindColor(index!.nodes.get(node.id())?.kinds[0] ?? 'package')); });
    cy.edges().forEach(edge => {
      const original = index!.outgoing.get(edge.source().id())?.find(candidate => `edge:${candidate.id}` === edge.id());
      edge.data('color', kindColor(original?.kinds[0] ?? 'package'));
    });
